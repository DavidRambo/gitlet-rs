...
** Related projects
   After doing it in Java, I implemented it in Python [[https://github.com/DavidRambo/gitlepy][Gitlepy]].
...
* Usage
  ...
  #+end_src
  creates the branch but remain in the currently checked out one; whereas
  #+begin_src:
...
* TODOs
  ...
  - [-] merge (in progress on branch ~dev_merge~)
     - [X] merge: fast-forward
     - [X] merge: no conflicts
     - [ ] merge: conflicts
